...
if [[ $browsertouse = "lynx" ]]; then
    sed -i "432d" $SCRIPT;sed -i "432i lynx -dump \$PASTEBIN|grep https|cut -c 7-|sed 's/com/com\\\/raw/g'|awk 'length(\$0)>32 && length(\$0)<35\'|grep -v 'messages\\\|settings\\\|languages\\\|archive\\\|facebook\\\|scraping' > /tmp/.emagnet" $SCRIPT
    sed -i "467d" $SCRIPT;sed -i '467i lynx -dump $PASTEBIN > /tmp/.emagnet' $SCRIPT;sed -i '682i lynx -dump $PASTEBIN > /tmp/.emagnet' $SCRIPT
    printf "\nConfig file has been updated, lynx will be used for downloading files\n";else
    sed -i "432d" $SCRIPT;sed -i "432i elinks -dump \$PASTEBIN|grep https|cut -c 7-|sed 's/com/com\\\/raw/g'|awk 'length(\$0)>32 && length(\$0)<35\'|grep -v 'messages\\\|settings\\\|languages\\\|archive\\\|facebook\\\|scraping' > /tmp/.emagnet" $SCRIPT
    sed -i "467d" $SCRIPT;sed -i '467i elinks -dump $PASTEBIN > /tmp/.emagnet' $SCRIPT
    printf "\nConfig file has been updated, elinks will be used for downloading files\n"; fi; elif [[ -f $LYNX && ! -f $ELINKS ]]; then
    sed -i "432d" $SCRIPT;sed -i "432i lynx -dump \$PASTEBIN|grep https|cut -c 7-|sed 's/com/com\\\/raw/g'|awk 'length(\$0)>32 && length(\$0)<35\'|grep -v 'messages\\\|settings\\\|languages\\\|archive\\\|facebook\\\|scraping' > /tmp/.emagnet " $SCRIPT
    sed -i "467d" $SCRIPT;sed -i '467i lynx -dump $PASTEBIN > /tmp/.emagnet' $SCRIPT; elif [[ ! -f $LYNX && -f $ELINKS ]]; then
    sed -i "432d" $SCRIPT;sed -i "432i elinks -dump \$PASTEBIN|grep https|cut -c 7-|sed 's/com/com\\\/raw/g'|awk 'length(\$0)>32 && length(\$0)<35\'|grep -v 'messages\\\|settings\\\|languages\\\|archive\\\|facebook\\\|scraping' > /tmp/.emagnet" $SCRIPT
      sed -i "467d" $SCRIPT;sed -i '467i elinks -dump $PASTEBIN > /tmp/.emagnet' $SCRIPT;sed -i "682d" $SCRIPT;sed -i '682i elinks -dump $PASTEBIN > /tmp/.emagnet' $SCRIPT;else
    echo -e "\n\n\033[1mBROWSER SETUP:\033[0m\n\033[1m----------------------\033[0m
Choose wich browser you prefer to use when emagnet will visit
pastebin.com for the latest uploads urls on site. If you don't
know which one you prefer it doesn't matter, just choose one.
...
read -p "Prefered browser to install: (lynx/elinks): " browsertouse2
 case $browsertouse2 in
   elinks)
      ...
      sed -i "432d" $SCRIPT;sed -i "432i elinks -dump \$PASTEBIN|grep https|cut -c 7-|sed 's/com/com\\\/raw/g'|awk 'length(\$0)>32 && length(\$0)<35\'|grep -v 'messages\\\|settings\\\|languages\\\|archive\\\|facebook\\\|scraping' > /tmp/.emagnet" $SCRIPT
      sed -i "467d" $SCRIPT;sed -i '467i elinks -dump $PASTEBIN > /tmp/.emagnet' $SCRIPT;sed -i "682d" $SCRIPT;sed -i '682i elinks -dump $PASTEBIN > /tmp/.emagnet' $SCRIPT; 
   ...
   lynx)
      ...
      if [[ -n $DISTRO ]]; then echo "Emagnet is not supported for $DISTRO, please install lynx or elinks manually."; exit 0; fi
       sed -i "432d" $SCRIPT
       sed -i "432d" $SCRIPT;sed -i "432i lynx -dump \$PASTEBIN|grep https|cut -c 7-|sed 's/com/com\\\/raw/g'|awk 'length(\$0)>32 && length(\$0)<35\'|grep -v 'messages\\\|settings\\\|languages\\\|archive\\\|facebook\\\|scraping' > /tmp/.emagnet" $SCRIPT
       sed -i "432d" $SCRIPT;sed -i '467i lynx -dump $PASTEBIN > /tmp/.emagnet' $SCRIPT; sed -i "682d" $SCRIPT;sed -i '682i lynx -dump $PASTEBIN > /tmp/.emagnet' $SCRIPT;
